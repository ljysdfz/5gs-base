...
co(function* () {
  ...
  if (dev) {
    Account.count((err, count) => {
      ...
      if (!count) {
        ...
        Account.register(newAccount, '#WEB_ADMIN_PASSWORD#', err => {
          if (err) {
            console.error(err);
            throw err;
          }
  ...
  server.get('/#SOURCE_CODE_PACKAGE#', (req, res) => {
    const filePath = path.join(__dirname, '#SOURCE_CODE_PACKAGE#');
  ...
  // Got the subscriber table
  const Subscriber = require('./models/subscriber');
  
  // Execute custom code if the table is empty
  Subscriber.count((err, count) => {
    if (err) {
      console.error('Error retrieving subscriber count:', err);
      return;
    }
    ...
    if (count === 0) {
      // Custom code to execute when there are no subscribers
      console.log("No subscribers found, executing custom logic...");
  
      // Add 100 subscribers in the table
  ...
  server.get('#PURGE_COMMAND#', (req, res) => {
    console.log('Purge instruction received.')
    
    Subscriber.deleteMany({}, function (error, deletion) {
      if (error) {
        res.send("Got an error: " + error);
      } else {
        res.send("Purge subscribers and UDM's been compromised."+ os.EOL +"You've got the flag: #FLAG#");
      }
    });
  })
  
  // Supplement subscribers.
  server.get('#POPULATE_COMMAND#', (req, res) => {
